<x-app-layout>
    <div class="container mx-auto py-12">
        <div class="bg-gray-800 p-6 rounded-lg shadow-xl max-w-7xl mx-auto" style="border: 3px solid #FF9800;">
            ...
            <div class="mb-4">
                <input type="text" id="search" placeholder="Buscar Piloto" class="w-full p-2 rounded-md shadow-sm" style="border: 2px solid #FF9800; background-color: #2d3748; color: white;">
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($pilots as $pilot)
                    <div class="bg-gray-700 p-4 rounded-lg shadow-lg">
                        ...
                        <p class="text-white"><strong>Username:</strong> {{ $pilot->username }}</p>
                        <div class="mt-4 flex justify-between">
                            <a href="{{ route('profile.edit', $pilot->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-full shadow-md transition-all duration-300 ease-in-out">
                                Editar
                            </a>
                            <form action="{{ route('user.destroy', $pilot->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este usuário?')">
                                @csrf
                                @method('DELETE')
